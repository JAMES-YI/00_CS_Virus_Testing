...
% performed; 
% - trialInd (which is actually the number of experiments) and the trialNum (which is actually the number of runs) are completely different;
% - totally 2 independent trial experiments are conducted for
% MHV-1; 
% - totally 1 independent trial experiment is done for COVID-19
...
switch config.virusID
    case 'MHV-1'
        ...
        MatSizeList = [3,7;...
                       4,15;...
                       5,31]; % each row is a matrix size
    case 'COVID-19'
        
        MatSizeList = [16,40]; % each row is a matrix size
end

solverList = {'EXHAUSTIVE', 'OBO_MM', 'MISMATCHRATIO_SUCC'}; % {'EXHAUSTIVE', 'OBO_MM', 'MISMATCHRATIO_SUCC'};
