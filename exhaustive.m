...
%                 skip = ~all(ismember(posPoolPatt,posMatPatt)) || sum(ismember(negPoolPatt,posMatPatt));
        ...
        switch exhaustMode
            ...
            case 'SUCCESSIVE'
                % - normalization is performed in a successive manner performing sign pattern coverage before solving
                %   successive weighted LSQ optimization; this is suggested by WXU, 11/02/2020
...
%                     cvx_begin quiet
%                         variable vloadSub(spst,1)
%                         minimize(norm( diag(1 ./ (sqrt(w)+epsilon)) * (MixMat(:,supp)*vloadSub-poolVload),2))
%                         subject to
%                             -vloadSub <= 0
%                     cvx_end
                    cvx_begin quiet
                        variable vloadSub(spst,1)
                        minimize(sum( diag(1 ./ (w+epsilon).^2) * (MixMat(:,supp)*vloadSub-poolVload).^2))
                        subject to
                            -vloadSub <= 0
                    cvx_end
